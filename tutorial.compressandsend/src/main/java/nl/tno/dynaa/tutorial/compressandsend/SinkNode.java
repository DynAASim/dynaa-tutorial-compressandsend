 ...
 * power consumed.
 *
 * In this file, we custom model a standard node to reflect a small and fictitious microprocessor.
 *
 */
public class SinkNode {
    ...
    public static Node build() throws Exception {
        ...
        final Quantity<ElectricCharge> batteryCharge = Quantities.getQuantity(7200.0, Units.COULOMB);
        ...
        final CommunicationDevice communicationDevice = new CommunicationDevice("COMM_DEVICE",
                communicationModes,
                "IDLE");
         ...
         */
        final Node sinkNode = new Node("SinkNode", processor, memory, battery);
        sinkNode.addPeripheral(communicationDevice);
